...
class PyTestRailPlugin(object):
    ...
    def pytest_collection_modifyitems(self, session, config, items):
        ...
        if self.testplan_id and self.is_testplan_available():
            ...
            if self.testrun_name is None:
                self.testrun_name = testrun_name(self.suite_id)
    ...
    def clean_output(self):
        path = os.path.abspath("tests/output")
    ...
    def pytest_sessionfinish(self, session, exitstatus):
        ...
        if self.results:
            ...
            if self.testrun_id:
                ...
                for item in range(0, len(self.response_id)):
                    self.add_attachment_for_run(self.response_id[item], f"{path}/{videos[item]}")
                self.clean_output()
                ...
                for testrun_id in testruns:
                    ...
                    # no testing
                    # video_name = session.items[0].module.pytest.video_name
                    # if video_name:
                    #     self.add_attachment_for_run(self.result_id, video_name)
            else:
    ...
    def get_failed_videos(self):
        videos = [x for x in os.listdir('tests/output') if x.endswith(".mov")]
        videos.sort()
        return videos
        ...
        if response:
            for x in range(0, len(self.results)):
                if self.results[x]['status_id'] == 5:
                    self.response_id.append(response[x]['id'])
